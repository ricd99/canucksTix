...
def getLocations(locationQuery):
    ...
    requestPayload = {
        "variables": """{"params": {"caller": "MARKETPLACE", "page_category": ["CITY", "SUBCITY", "NEIGHBORHOOD","POSTAL_CODE"], "query": "%s"}}"""
        % (locationQuery),
...
def getListings(locationLatitude, locationLongitude, listingQuery, numPageResults=1):
    ...
    if status == "Success":
        ...
        with open("debug_response.json", "w") as f:
            json.dump(facebookResponseJSON, f, indent=2)
        print(facebookResponseJSON)
        ...
        for _ in range(1, numPageResults):
            ...
            if pageInfo["has_next_page"]:
                cursor = facebookResponseJSON["data"]["marketplace_search"][
                    "feed_units"
                ...
                requestPayloadCopy["variables"] = requestPayloadCopy[
                    "variables"
                ].split()
...
def getFacebookResponse(requestPayload):
    ...
    # Try making post request to Facebook, except return
    try:
    ...
    if facebookResponse.status_code == 200:
        ...
        status = "Failure"
        error["source"] = "Facebook"
        error["message"] = "Status code {}".format(facebookResponse.status_code)
        ...
        for listing in rawPageResult["data"]["marketplace_search"]["feed_units"][
            ...
            if listing["node"]["__typename"] == "MarketplaceFeedListingStoryObject":
                ...
                if listing["node"]["listing"]["strikethrough_price"]:
                    listingPreviousPrice = listing["node"]["listing"][
                        "strikethrough_price"
                    ]["formatted_amount"]
                else:
